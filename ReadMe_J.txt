...
(2) ファイル説明
  Sample/フォルダの中に下記のファイルが存在します。
    ...
    hvc_p2_api.cs                 B5T-007001 C# APIクラス（結果安定化後）
...
(3) サンプルコードのビルド方法
  1. 本サンプルコードは Widows 10/11 上で動作するよう作成しています。
     Visual Studio 2010以降のバージョンでコンパイルが可能です。
  2. コンパイル後は、Sample/bin/以下にexeファイルが生成されます。

(4) サンプルコードの実行方法
  実行には、exeファイルと同じディレクトリに、STBLibのDLLファイルが必要です。
  （あらかじめSample/bin/にlibSTB.dllは格納されています）

　1. exeファイルを実行するとフォーム画面が表示されます。
　2. フォーム画面がらB5T-007001が接続しているCOM番号を選択します。
　3-1. Executeボタンを押すと検出処理が実行されます。
　3-2. Registerボタンを押すと顔認証用データ登録が実行されます。
...
----
オムロン株式会社
Copyright(C) 2018-2025 OMRON Corporation, All Rights Reserved.
